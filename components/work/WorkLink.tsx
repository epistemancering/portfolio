...
export default function WorkLink(props: { route: string }) {
    ...
    return <>
        ...
        <Image className = {`${getDark ? "shadow-whitesmall hover:shadow-whitelarge" : "shadow-darksmall hover:shadow-darklarge"} rounded-xl lg:rounded-3xl h-[174px] lg:h-auto`} style = {{ display: "block", width: "100vw", maxWidth: "100%", objectFit: "cover", transitionDuration: "200ms", transitionProperty: "box-shadow" }} height = {568} width = {958} src = {`/work/${props.route}.png`} sizes = {"(min-width: 1024px) 640px, calc(100vw - 48px)"} quality = {53} alt = {`${projectRoutes[props.route].title} demonstration`} />
    </>
}